...
namespace TerrainSlicingKit
{
    ...
    public class atsColormapMaterialSlicer : SliceProcessor
    {
        ...
        //The master folder to save sliced materials and textures. Note that files will not be saved directly in this folder. Instead, each slicing job will create a sub folder within this folder, which will in turn contain sub folders for each individual terrain slice created by that slicing job.
        [SerializeField]
        ...
        void SliceMaterial(TerrainSlice slicedTerrain, Material materialToSlice, string applicationSavePath, string unitySavePath)
        {
            ...
            RefreshAssetDatabase();
            Material slicedMaterial = AssetDatabase.LoadAssetAtPath(fullAssetPathOfNewMaterial, typeof(Material)) as Material;
